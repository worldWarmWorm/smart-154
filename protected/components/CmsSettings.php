...
class CmsSettings extends CApplicationComponent
{
    ...
    {
        $icon_ext = $this->get('cms_settings', 'favicon');
        if ($icon_ext) {
            return '/favicon.' . $icon_ext;
        }

        $theme = Yii::app()->getTheme();
        if (is_file($theme->basePath . DS . 'favicon.png')) {
            return $theme->baseUrl . '/favicon.png';
        }
        if (is_file($theme->basePath . DS . 'favicon.ico')) {
            return $theme->baseUrl . '/favicon.ico';
        }

        $webroot = Yii::getPathOfAlias('webroot');
        if (is_file($webroot . DS . 'favicon.png')) {
            return '/favicon.png';
        }
